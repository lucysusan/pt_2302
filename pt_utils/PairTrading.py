...
pair_pct_bar = config['formation'].getfloat('pair_pct_bar')
...
lc_rate = config['transaction'].getfloat('lc_rate')
...
def PairTrading_once(form_end=form_end, form_freq=form_freq, form_freq_num=form_freq_num, project_path=project_path,
                     ...
                     trans_freq_num=trans_freq_num, lc_rate=lc_rate,
                     index_df=index_df, index_sid=index_sid, disp: bool = True):
    # today = get_current_date()
    trade_out_folder = f"""{project_path}/output/0302_loss/{form_freq_num}{str(form_freq).split('.')[-1]}_{trans_freq_num}{str(trans_freq).split('.')[-1]}/{form_end}_{trans_start}/"""
    ...
    trade = Trading(pair_entry_dict, pair_bar, trans_start, freq=trans_freq, freq_num=trans_freq_num, lc_rate=lc_rate,
                    out_folder=trade_out_folder + 'transaction/')
    if pair_entry_dict:
        rev_df = trade.run(trade_out_folder)
...
def PairTrading(start_date=start_date, end_date=end_date, form_freq=form_freq, form_freq_num=form_freq_num,
    ...
    while trans_start <= trans_start_end:
        ...
        bm_data, trans_start = PairTrading_once(form_end, form_freq, form_freq_num, project_path, pair_bar, c,
                                                pair_pct_bar,
                                                trans_start,
                                                trans_freq, trans_freq_num, lc_rate, index_df, index_sid, disp)
        bm_df = pd.concat([bm_df, bm_data])
        form_end = trans_start

    # today = get_current_date()
    out_folder = f"""{project_path}/output/0302_loss/"""
